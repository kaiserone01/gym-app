...
- [x] ~~Integración real de la API BCV~~ — Plan 7, completo y verificado contra la API y la base reales. Conectada a `/api/tasa-cambio`, pero **todavía no** a los formularios que la necesitan (Nuevo Miembro/Registrar Pago usan una tasa fija de 850 — ver "Ajustes ad-hoc" arriba).
- [ ] **Automatizar la actualización diaria de la tasa BCV — EN PAUSA a pedido explícito del usuario, no retomar sin que lo pida.** La tasa quedó desactualizada 11 días en producción (832.49 en vez de 852.41 real) porque nadie corría `npm run actualizar-tasa` a mano; se corrigió el valor manualmente una vez. Se creó `apps/worker/src/daemon.ts` (long-running, cron diario a las 12:00 UTC vía `node-cron`, script `worker:daemon`) para que no vuelva a pasar, pero el intento de desplegarlo como servicio nuevo en Easypanel ("tasa-bcv") se frenó: el servicio real `gym-app` en Easypanel usa un `.env` cifrado con **dotenvx** subido manualmente a la UI (no está en el repo, `.gitignore` tiene `.env*`), y no se encontró en la UI de Easypanel dónde se carga ese `.env` para replicarlo en el servicio nuevo. El servicio "tasa-bcv" quedó CREADO en Easypanel (Nixpacks, comando de inicio `npm run worker:daemon`) pero sin variables de entorno y sin desplegar. Detalle completo en `handoff.md` sección 2.1.
